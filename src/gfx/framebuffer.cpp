...
FrameBuffer::Builder& FrameBuffer::Builder::addAttachment(const gfx::Texture::Builder& builder, gfx::Texture::Type type, gfx::Texture::InternalFormat internalFormat, FrameBuffer::Attachment attachment, uint32_t mipLevel) {
    ...
    tex->create(info);
    textures[attachment] = {tex, mipLevel, true};
    return *this;
}

FrameBuffer::Builder& FrameBuffer::Builder::addTexture(std::shared_ptr<gfx::Texture>& texture, FrameBuffer::Attachment attachment, uint32_t mipLevel) {
    assert(!textures.contains(attachment));
    textures[attachment] = {texture, mipLevel, false};
    return *this;
}
...
FrameBuffer FrameBuffer::Builder::build() {
    assert(textures.size());  // cant be 0
...
FrameBuffer::FrameBuffer(GLuint id, std::map<Attachment, TextureMipLevelShouldResize>& textures, uint32_t width, uint32_t height) : id(id), textures(textures), width(width), height(height) {
...
void FrameBuffer::invalidate(uint32_t width, uint32_t height) {
    if (width != FrameBuffer::width || height != FrameBuffer::height) {
        ...
        for (auto& kv : textures) {
            if (!kv.second.shouldResize) continue;
            auto& textureMipResize = kv.second;
            auto& texture = textureMipResize.texture;
            auto& attachment = kv.first;
            auto& mipLevel = textureMipResize.mipLevel;
