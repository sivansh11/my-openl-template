...
std::shared_ptr<Texture> Texture::createDefaultTexture(Texture::Builder builder, uint8_t defaultValue) {
    uint8_t *data = new uint8_t[4];
    for (int i = 0; i < 4; i++) data[i] = defaultValue;
    std::shared_ptr<gfx::Texture> tex = builder.build(gfx::Texture::Type::e2D);
