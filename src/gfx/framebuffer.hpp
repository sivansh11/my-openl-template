...
public:
    ...
    struct TextureMipLevelShouldResize {
    ...
    struct Builder {
        std::map<Attachment, TextureMipLevelShouldResize> textures;
        ...
        Builder& addTexture(std::shared_ptr<gfx::Texture>& texture, Attachment attachment, uint32_t mipLevel = 0);
    ...
    FrameBuffer() = default;
    FrameBuffer(GLuint id, std::map<Attachment, TextureMipLevelShouldResize>& textures, uint32_t width, uint32_t height);
    ~FrameBuffer();
